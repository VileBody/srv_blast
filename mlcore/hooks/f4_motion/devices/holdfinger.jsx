/**********************************************************************
 * F4 motion-hook overlay — device "holdfinger"  ("задержи палец над кругом")
 * Injectable form of rebuild_holdfinger.jsx. Layers built on the passed comp
 * from comp-time 0 (clip window pre-reframed so cover-end == hook).
 * Tokens: __F4_BPM__, __F4_DEVICE__. Layer length FIXED (timeScale=1.0).
...
(function (comp) {
  ...
  var CONFIG = {
    textHold:    "ЗАДЕРЖИ ПАЛЕЦ\rНАД КРУГОМ",
  ...
  function buildBpmControl(comp){
    ...
    try { var sl = fx.addProperty("ADBE Slider Control"); sl.property("ADBE Slider Control-0001").setValue(CONFIG.bpm); } catch(e){}
    return L;
  }

  function buildSag1(comp){
    var sub = app.project.items.addComp("sag1 белый",
    ...
    setEffParam(ramp, "ADBE Ramp-0002", [1,0,0]);
    setEffParam(ramp, "ADBE Ramp-0003", [740,960]);
    setEffParam(ramp, "ADBE Ramp-0004", [1,1,1]);
    setEffParam(ramp, "ADBE Ramp-0005", 1);
    setEffParam(ramp, "ADBE Ramp-0007", 0);

    var ring = sub.layers.addShape();
    ring.name = "ring";
    var rroot = ring.property("ADBE Root Vectors Group");
    var rgrp  = rroot.addProperty("ADBE Vector Group"); rgrp.name = "Эллипс 1";
    var rcont = rgrp.property("ADBE Vectors Group");
    var ell = rcont.addProperty("ADBE Vector Shape - Ellipse");
    setConst(ell.property("ADBE Vector Ellipse Size"),     [400,400]);
    setConst(ell.property("ADBE Vector Ellipse Position"), [0,0]);
    var st = rcont.addProperty("ADBE Vector Graphic - Stroke");
    setConst(st.property("ADBE Vector Stroke Color"), [1,1,1,1]);
    setConst(st.property("ADBE Vector Stroke Width"), 20);
    var rtr = ring.property("ADBE Transform Group");
    setConst(rtr.property("ADBE Anchor Point"), [0,0,0]);
    setConst(rtr.property("ADBE Position"),     [540,960,0]);
    ...
    try { grad.trackMatteType = TrackMatteType.ALPHA; }
    catch(e){ try { grad.setTrackMatte(ring, TrackMatteType.ALPHA); } catch(e2){} }
  ...
  function buildHoldText(comp){
    ...
    L.name = "ЗАДЕРЖИ ПАЛЕЦ НАД КРУГОМ";
    L.startTime = 0; L.inPoint = 0; L.outPoint = t(5.30530530530531);
  ...
  app.beginUndoGroup("F4 holdfinger overlay");
  try {
