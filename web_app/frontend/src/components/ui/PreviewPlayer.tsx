  ...
  return (
    <div className={cn('group/preview relative overflow-hidden', className)} style={style}>
      ...
      {showSteps && onPrev && onNext && (
        <>
          <button type="button" className={cn(arrow, controlsVisibility, 'left-space-4')} aria-label={t('common.prev')} onClick={onPrev}>
            <svg viewBox="0 0 24 24" width="22" height="22" fill="none" aria-hidden="true">
              <path d="M14.5 6 8.5 12l6 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          <button type="button" className={cn(arrow, controlsVisibility, 'right-space-4')} aria-label={t('common.next')} onClick={onNext}>
            <svg viewBox="0 0 24 24" width="22" height="22" fill="none" aria-hidden="true">
      ...
      {onTogglePlay && (
        <button
          ...
          className={cn('absolute left-1/2 top-1/2 z-[4] flex h-[64px] w-[64px] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-[rgba(5,1,15,0.6)] text-text backdrop-blur-[8px] transition-[opacity,filter] hover:brightness-125', controlsVisibility)}
        >
          {playing
