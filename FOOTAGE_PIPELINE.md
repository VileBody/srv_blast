...
4. **Разметить новые клипы** (Ось 2): кнопка «🏷 Разметить» в admin UI (тегает только untagged) → пишет в `footage_tags`.
5. **Пере-экспортировать снапшот** (чтобы пикер увидел новые теги):
   ```bash
...
- ⏳ #2 Курация админки (exclude/blacklist/assign) пишется в файлы в контейнере asset-ui (без `./data`-маунта) → **не доходит до пикера**. Удаление работает только через S3-trash + пересборку индекса. Чинить переносом в Postgres.
...
- ⏳ #5 Снапшот обновляется вручную (шаг 5). Кандидат на автоэкспорт после разметки.
