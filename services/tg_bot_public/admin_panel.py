...
from .state_store import STAGE_IDLE
...
def build_app(
    ...
    async def users_list(request: Request, _user: str = Depends(_check_auth)) -> str:
        ...
        # Get current stages from Redis — parallel per-user lookups instead of full SCAN
        states = await asyncio.gather(*[state_store.get(u["tg_id"]) for u in users])
        stages_map: dict[int, str] = {}
        for u, st in zip(users, states):
            if st.stage != STAGE_IDLE:
                stages_map[u["tg_id"]] = st.stage
    ...
    async def user_detail(tg_id: int, _user: str = Depends(_check_auth)) -> str:
        ...
        for t in txs:
            ...
            reason = t["reason"]
            reason_cell = html_mod.escape(reason)
            if reason in ("payment", "admin_activate", "initial_grant", "referral"):
                reason_cell = f'<span class="badge badge-stage">{html_mod.escape(reason)}</span>'
            tx_rows += (
                f"<tr><td>{t['id']}</td><td>{sign}{t['amount']}</td>"
                f"<td>{reason_cell}</td><td>{html_mod.escape(str(t['admin_note']))}</td><td>{t['created_at']}</td></tr>"
            )

        # Payments
        pays = await credits_db.get_payments(tg_id, limit=50)
        pay_rows = ""
        for p in pays:
            status_cls = "badge-ok" if p["status"] == "CONFIRMED" else "badge-zero"
            utm_parts = [p.get("utm_source", ""), p.get("utm_campaign", "")]
            utm_str = html_mod.escape(" / ".join(x for x in utm_parts if x)) or '<span style="color:#ccc">—</span>'
            pay_rows += (
                f"<tr><td>{p['order_id']}</td><td>{p['amount_rub']}&rub;</td>"
                f"<td>{p['package']}</td>"
        ...
        <form method="post" action="/admin/users/{tg_id}/activate" onsubmit="return confirm('Активировать пакет для ' + {json.dumps(html_mod.escape(uname))} + '?')">
        ...
        <h3>Платежи</h3>
        <div class="table-wrap">
        <table><tr><th>Order</th><th>Amount</th><th>Package</th><th>Status</th><th>UTM</th><th>Date</th></tr>
        {pay_rows if pay_rows else '<tr><td colspan="6">Нет данных</td></tr>'}</table>
        </div>
        </div>
    ...
    async def transactions_list(request: Request, _user: str = Depends(_check_auth)) -> str:
        ...
        for t in txs:
            sign = "+" if t["amount"] > 0 else ""
            reason = t["reason"]
            note = html_mod.escape(str(t["admin_note"]))
            # Show who granted and link to user
            reason_cell = html_mod.escape(reason)
            if reason in ("payment", "admin_activate", "initial_grant", "referral"):
                reason_cell = f'<span class="badge badge-stage">{html_mod.escape(reason)}</span>'
            rows += (
                f"<tr><td>{t['id']}</td>"
                f"<td><a href='/admin/users/{t['tg_id']}'>{t['tg_id']}</a></td>"
                f"<td>{sign}{t['amount']}</td>"
                f"<td>{reason_cell}</td><td>{note}</td><td>{t['created_at']}</td></tr>"
            )
        body = f"""
        <div class="card">
        <p>Total: {total}</p>
    ...
    async def payments_list(request: Request, _user: str = Depends(_check_auth)) -> str:
        ...
        for p in pays:
            status_cls = "badge-ok" if p["status"] == "CONFIRMED" else "badge-zero"
            utm_src = html_mod.escape(p.get("utm_source") or "")
            utm_cmp = html_mod.escape(p.get("utm_campaign") or "")
            rows += (
                f"<tr><td>{p['id']}</td>"
        ...
        <p>Total: {total}</p>
        <div class="table-wrap">
        <table><tr><th>#</th><th>tg_id</th><th>Order</th><th>Amount</th><th>Package</th><th>Status</th><th>UTM Source</th><th>Campaign</th><th>Date</th></tr>
        {rows}</table>
        </div>
        {_pagination_html(page, total_pages)}
        </div>
    ...
    async def utm_summary(_user: str = Depends(_check_auth)) -> str:
        ...
        for row in rows_data:
            rows += (
                f"<tr>"
                f"<td>{html_mod.escape(row['source'] or '(none)')}</td>"
                f"<td>{html_mod.escape(row['medium'] or '(none)')}</td>"
                f"<td>{html_mod.escape(row['campaign'] or '(none)')}</td>"
                f"<td>{html_mod.escape(row.get('content') or '')}</td>"
                f"<td>{html_mod.escape(row.get('term') or '')}</td>"
