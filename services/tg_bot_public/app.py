...
class BlastBotApp:
    def __init__(self, settings: Settings):
        ...
        self.credits_db = CreditsDB(settings.credits_db_path)
    ...
    async def _move_to_wait_audio(self, chat_id: int, message: Message) -> None:
        ...
        await message.answer("Пришли аудио в формате mp3.")
    ...
    async def _handle_wait_audio(self, message: Message, st: ChatState) -> None:
        ...
        if text in (BTN_SEND_TRACK, BTN_GENERATE_MORE):
            await message.answer("Жду аудио-файл в формате mp3.", reply_markup=ReplyKeyboardRemove())
            return

        spec = _extract_audio_spec(message)
        if spec is None:
            await message.answer("Нужен аудио-файл в формате mp3. Нажми «Отправить трек» и пришли файл.")
            return

        # Check mp3 format
        _, original_name = spec
        if not str(original_name).lower().endswith(".mp3"):
            await message.answer("Пока бот принимает только mp3. Пришли файл в формате .mp3.")
            return
    ...
    async def _show_purchase_stub(self, message: Message, st: ChatState) -> None:
        ...
        if self.tbank and price > 0:
            ...
            try:
                await self.credits_db.create_payment(order_id, st.chat_id, price, pkg)
