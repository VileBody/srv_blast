...
def test_build_job_openrouter_retries_on_gateway_timeout_524(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    job_id = "job_openrouter_gateway_timeout_524_retry"
    store = _FakeStore(
        job_id=job_id,
        request={
            "audio_s3_url": "s3://bucket/raw/audio.mp3",
    ...
    kwargs = retry_calls[0]["kwargs"]
    assert float(kwargs["countdown"]) == 10.0
    assert "openrouter_gateway_timeout_524" in str(kwargs["exc"])
    assert dispatch_calls == []
