                ...
                <div className="pill-popover" ref={popoverRef}>
                    ...
                    <div className="popover-assignments">
                      {status!.assigned_to!.map((a, i) => (
                        <div key={i} className="popover-assignment-row">
                          <span className="assignment-label">{a.theme} / {a.group}</span>
                          <button
                  ...
                  {!assignStep && (
                    <div className="popover-actions">
                      {isBlacklisted ? (
                        <button className="btn-popover btn-restore" onClick={() => handleUnblacklist(tag)} disabled={saving}>
                          Восстановить
                        </button>
                      ) : (
                        <>
                          <button className="btn-popover btn-add-theme" onClick={() => setAssignStep('pick-theme')} disabled={saving}>
                            Добавить в тему
                          </button>
                          <button className="btn-popover btn-blacklist" onClick={() => handleBlacklist(tag)} disabled={saving}>
                            Удалить
                          </button>
                        </>
                      )}
                  ...
                  {assignStep === 'pick-theme' && (
                    <div className="popover-picker">
                      ...
                      <div className="picker-list">
                        {themeNames.map((name) => (
                          <button key={name} className="picker-item" onClick={() => { setSelectedTheme(name); setAssignStep('pick-group'); }}>
                            {name.replace(/_/g, ' ')}
                          </button>
                        ))}
                      </div>
                  ...
                  {assignStep === 'pick-group' && selectedTheme && (
                    <div className="popover-picker">
                      ...
                      <div className="picker-list">
                        {groupNames.map((gn) => (
                          <button key={gn} className="picker-item" onClick={() => handleAssign(tag, selectedTheme, gn)} disabled={saving}>
                            {gn.replace(/_/g, ' ')}
                          </button>
                        ))}
                      </div>
