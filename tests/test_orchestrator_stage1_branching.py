...
def test_non_legacy_uses_stage1a_selected_fragment_and_skips_stage1b(
    ...
    assert [t.text for s in plan.subtitles.segments for t in s.tokens] == ["hello", "world"]
...
def test_forced_alignment_retries_when_user_clip_window_has_no_words(
    monkeypatch,
    tmp_path: Path,
) -> None:
    audio_path = tmp_path / "audio.mp3"
    ...
    monkeypatch.setenv("AUDIO_FILE_PATH", str(audio_path))
    monkeypatch.setenv("AUDIO_DIR", str(audio_path.parent))
    monkeypatch.setenv("JOB_ID", "job_forced_alignment_retry_empty_user_clip")
    monkeypatch.setenv("SUBTITLES_MODE", "impulse_2nd")
    monkeypatch.setenv("STAGE2_TIMING_MODE", "prompts")
    monkeypatch.setenv("STAGE2_FAST_START_SECONDS", "1")
    monkeypatch.setenv("LYRICS_TEXT", "hello world")
    monkeypatch.setenv("USER_CLIP_START_SEC", "10")
    monkeypatch.setenv("USER_CLIP_END_SEC", "12")

    monkeypatch.setattr(go, "_make_client", lambda **kwargs: object())
    monkeypatch.setattr(go, "pick_audio_files", lambda _audio_dir: [audio_path])
    monkeypatch.setattr(go, "detect_bpm_librosa", lambda **kwargs: 120.0)

    bad_alignment = Stage1ForcedAlignmentPayload.model_validate(
        {
            "aligned_words": [
                {"text": "hello", "t_start": "00:00.100", "t_end": "00:00.500"},
                {"text": "world", "t_start": "00:00.700", "t_end": "00:01.100"},
            ],
            "pause_spans": [],
        }
    )
    good_alignment = Stage1ForcedAlignmentPayload.model_validate(
        {
            "aligned_words": [
                {"text": "hello", "t_start": "00:10.100", "t_end": "00:10.500"},
                {"text": "world", "t_start": "00:10.700", "t_end": "00:11.100"},
            ],
            "pause_spans": [],
        }
    )

    calls = {"stage1_forced": 0}

    def _call_stage1_forced_with_two_empty_clip_attempts(**kwargs):
        calls["stage1_forced"] += 1
        if calls["stage1_forced"] < 3:
    ...
    monkeypatch.setattr(
        ...
        _call_stage1_forced_with_two_empty_clip_attempts,
    )
    monkeypatch.setattr(
        go,
        "call_stage1_scenario_once",
    ...
    out = go.build_all_via_gemini_one_call()
    assert set(out.keys()) == {"audio_plan", "full_edit_config", "footage_config"}
    assert calls["stage1_forced"] == 3
    plan = captured["plan"]
    assert abs(float(plan.audio.clip_start_abs) - 10.0) < 1e-6
    assert abs(float(plan.audio.clip_end_abs) - 12.0) < 1e-6
