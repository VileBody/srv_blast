...
class BlastBotApp:
    ...
    async def _enqueue_batch_version(
        ...
        *,
        ...
        # Deterministic key: (chat_id, batch_id, version) — retries hit the same slot.
    ...
    async def _process_chat_job(self, st: ChatState) -> None:
        ...
        if can_enqueue_more:
            if master_status == "FAILED":
                # master (text) job failed — don't attempt further versions.
                await bot.send_message(
                    st.chat_id,
        ...
        enqueue_short = enqueued_count < total_versions
