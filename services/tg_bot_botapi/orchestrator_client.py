...
class OrchestratorClient:
    ...
    async def rank_buckets(
        ...
        *,
        ...
        payload = {
            ...
            # Which catalog plane to rank: "vibes" is the semantic shortlist,
            # the others are folder-scoped collections (films / people /
            # cine16x9). Orthogonal to media_type.
            "pool": str(pool or "vibes").strip() or "vibes",
        }
    ...
    async def analyze_hook(
        ...
        *,
        ...
        """F4 «Движение» picker: ask the orchestrator (it has librosa) to run the
        focus-clip analysis and return {bpm, drop_candidates}. Keeps librosa out
        of the slim bot image."""
        payload = {
            "audio_s3_url": str(audio_s3_url),
    ...
    async def kill_job(self, job_id: str, *, reason: str = "") -> Dict[str, Any]:
        """Best-effort kill of a RUNNING/QUEUED job (used by the /bigtest
        safety-breaker to stop a case that re-ran Stage1 ASR)."""
        jid = str(job_id or "").strip()
        if not jid:
