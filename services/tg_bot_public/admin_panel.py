...
def build_app(
    ...
    async def dashboard(_user: str = Depends(_check_auth)) -> str:
        ...
        utm_summary = await credits_db.get_utm_summary(limit=10)
        utm_rows = ""
        for row in utm_summary:
            src = row["source"] or "(none)"
            med = row["medium"] or "(none)"
            camp = row["campaign"] or "(none)"
            utm_rows += (
                f"<tr><td>{src}</td><td>{med}</td><td>{camp}</td>"
                f"<td>{row['starts_count']}</td><td>{row['paid_orders']}</td><td>{row['revenue_rub']}₽</td></tr>"
            )
        ...
        {utm_rows if utm_rows else '<tr><td colspan="6">Нет данных</td></tr>'}</table>
        ...
        if status == "CONFIRMED" and payment:
            ...
            if bot_ref and bot_ref[0]:
                try:
                    ...
                    await bot_ref[0].send_message(tg_id, "Пришли аудио-файл.")
