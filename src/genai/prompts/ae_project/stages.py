...
AE_SUBTITLES_STAGE = (
    ...
    "ВАЖНО ДЛЯ TAG-STYLE (СТРОГО):\n"
    "- Если у subtitle задан tag/tagId, ты ДОЛЖЕН добавить tagPlan.\n"
    "- tagPlan задаёт ТОЛЬКО тайминги ключей (мы применим их к пресетным keyframes).\n"
    "- Формат:\n"
    "  tagPlan = {\n"
    "    \"mode\": \"absolute\",\n"
    "    \"keyframes\": {\n"
    "      \"<canonicalObjectName>\": {\n"
    "        \"<slug_time>\": [t0, t1, ...]   // абсолютные секунды ВНУТРИ КОМПОЗИЦИИ (та же ось, что inPoint/outPoint)\n"
    "      }\n"
    "    },\n"
    "    \"words\": [...] // обязателен ТОЛЬКО если requires_words=true для этого tagId\n"
    "  }\n"
    "- <slug_time> совпадает со slug из tag-pack keyframes (он выглядит как \"..._time\").\n"
    "- Кол-во времен в массиве ДОЛЖНО совпадать с количеством ключей в этом параметре пресета.\n"
    "- Никаких t(0..1) — только absolute.\n\n"
    "Правила разметки:\n"
