...
async def get_server(token: str, server_id: int) -> dict[str, Any]:
    async with httpx.AsyncClient(timeout=30.0) as client:
        data = await _json_request(
            client,
            "GET",
            f"/servers/{int(server_id)}",
            token=token,
            expected_codes=(200,),
        )
    server_obj = data.get("server")
    if not isinstance(server_obj, dict):
        raise RenderNodePoolError(f"GET /servers/{server_id} returned invalid payload")
    return server_obj
    ...
    *,
    ...
    for row in raw:
        ...
        if include_ids and server_id in include_ids:
            ...
            # Without explicit filters this endpoint can return unrelated infra.
            continue
...
async def create_render_server_from_clone(
    *,
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        # Rename for operational clarity.
        await _json_request(
            client,
            "PATCH",
            f"/servers/{server_id}",
        ...
        # Clone can stay in OFF after cloning completes; explicitly start.
        await _json_request(
            client,
            "POST",
...
async def delete_render_server(
    *,
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        if firewall_group_id:
            # Best-effort detach: Timeweb can keep stale references for removed servers.
            await _json_request(
                client,
                "DELETE",
