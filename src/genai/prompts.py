...
AE_COMPOSITION_STAGE = (
    ...
    '          "animId": "anim_reveal_opacity", "transformId": "tf_subtitle_base",\n'
    '          "animParams": { "kind": "reveal_ramp_abs", "t_start": 1.88, "t_end": 2.20 },\n'
    '          "transformParams": { "kind": "opacity_fade_abs", "t_start": 2.90, "t_end": 3.04, "from": 100, "to": 0 }\n'
    '        },\n'
    '        { "type": "text", "styleId": "highlight_subtitle", "content": "she was three",    "inPoint": 3.044, "outPoint": 4.379,\n'
    '          "animId": "anim_reveal_opacity", "transformId": "tf_subtitle_base",\n'
    '          "animParams": { "kind": "reveal_ramp_abs", "t_start": 3.05, "t_end": 3.45 }\n'
    '        },\n'
    ...
    'Ограничения по анимации/оверрайдам:\n'
    '- Для текста ты выбираешь animId/transformId из палитры и подгоняешь их ПО ВРЕМЕНИ.\n'
    '- Предпочтительный способ: НЕ писать низкоуровневые keyframes, а задавать animParams/transformParams.\n'
    '- Все времена (t_start/t_end/...) — абсолютное время композиции (секунды).\n'
    '- overrides.* (raw keys/procedural) оставлены для обратной совместимости, но старайся их НЕ использовать.\n'
...
AE_PROJECT_FOOTER = (
    ...
    "- В items ОБЯЗАТЕЛЬНО добавь аудио-элемент (без вариантов):\n"
    '  { "id": "audio_main", "type": "footage", "name": "Audio Track", "path": "media/audio/track.m4a", "isRef": true }\n'
    "- ВАЖНО: поле path у audio_main обязано быть именно 'media/audio/track.m4a'.\n"
    "- НЕ добавляй в audio_main поля композиций (layers/duration/fps/width/height/pixelAspect и т.п.).\n"
