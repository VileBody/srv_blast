/*** invert_flash (transition) — ч/б пульс-инверт на склейках. Авто-каркас ***/
...
function one(comp,t0,ref){
  var L=comp.layers.addSolid([1,1,1],"invert flash",comp.width,comp.height,1); L.adjustmentLayer=true; L.startTime=0; L.inPoint=t0; L.outPoint=t0+CONFIG.duration;
  var fx=L.property("ADBE Effect Parade");
  fx.addProperty("ADBE Invert");
  try{ var lu=fx.addProperty("ADBE Lumetri"); setP(lu,"ADBE Lumetri-0020",0); }catch(e){} // Saturation 0 -> ч/б
  var n=fx.addProperty("ADBE Noise2"); setP(n,"ADBE Noise2-0001",30); setP(n,"ADBE Noise2-0002",0);
...
(function(){ if(!app.project){log("нет проекта");return;} var comp=findComp(); if(!comp){log("нет компа");return;}
  app.beginUndoGroup("invert flash"); try{ var ts=times(),ref=refLayer(comp); for(var i=0;i<ts.length;i++)one(comp,ts[i],ref); log("invert flash x"+ts.length+" -> "+comp.name);}catch(e){log("err "+e+" "+(e.line||""));} finally{app.endUndoGroup();} })();
